        ...
        try:
            with transaction.atomic():  # 트랜잭션
                if item_in_id is None:
                    ...
                    item.current_quan += quantity_difference
                    item.save()
                    ...
                    # StockStatus 업데이트
                    stock_status = StockStatus.objects.get(input_id=item_in_id)
                    stock_status.quantity = float(table_data['in_quan'])
...
class ItemInFilter(View):
    def get(self, request, *args, **kwargs):
        ...
        if product_name_num:
            qs = qs.filter(
                Q(in_item__item_code__icontains=product_name_num) |
                Q(in_item__item_name__icontains=product_name_num)
            )
        ...
        if location_input:
            qs = qs.filter(
                Q(in_no__icontains=location_input) |
                Q(in_custom__c_name__icontains=location_input)
            )
